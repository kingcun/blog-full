...
export async function fixTopField(type: "local" | "remote", db: string, infoExists: boolean) {
  ...
  const result = await runWranglerJson([
    ...
    "--json",
    "--command",
    "SELECT name FROM pragma_table_info('posts') WHERE name='top'",
  ]);

  if (result[0].results.length === 0) {
    console.log("Adding top field to posts table");
    await runWranglerQuiet([
      ...
      "--json",
      "--command",
      "ALTER TABLE posts ADD COLUMN top INTEGER DEFAULT 0",
    ]);
  } else {
    console.log("Top field already exists in posts table");
  }
}
