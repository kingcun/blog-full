...
**旧的方式（不推荐）：**
```typescript
group.get('/post/:id', async (ctx: Context) => {
    const { id } = ctx.params;
    ...
    if (!post) {
        ctx.set.status = 404;
        return 'Post not found';  // 不一致的错误格式
    }
...
group.get('/post/:id', async (ctx: Context) => {
    ...
    if (!post) {
        throw new NotFoundError('Post');  // 自动返回结构化错误
    }
    
    if (post.draft && !ctx.admin) {
...
function PostList() {
  const { data: posts, loading, error, execute } = useAsync(
    () => client.post.list(),
    {
      immediate: true,
      onError: (err) => {
...
function CreatePost() {
  const { execute: createPost, loading, error } = useApi(
    (data) => client.post.create(data),
    {
      onSuccess: () => {
        // 显示成功消息
      },
...
1. **替换 Promise.catch：**
   ...
   // 新
   const { data, error } = useApi(() => client.post.list(), { immediate: true });
   ```
