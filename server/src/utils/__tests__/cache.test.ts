...
describe('CacheImpl - 数据库持久化测试', () => {
    ...
    it('clear 不应该清除其他类型的数据（如 server.config 和 client.config）', async () => {
        ...
        // 设置数据
        await publicCache.set('post_1', 'post_data');
...
describe('CacheImpl - 公共缓存开关', () => {
    ...
    it('默认关闭时不读取或写入公共缓存', async () => {
        ...
        await seededCache.set('post_1', { title: 'cached' });
        cacheImpl = new CacheImpl(db as any, mockEnv, 'cache', 'database', clientConfig);

        expect(await cacheImpl.get('post_1')).toBeNull();

        let computed = false;
        const value = await cacheImpl.getOrSet('post_1', async () => {
            computed = true;
            return { title: 'fresh' };
        });
        ...
        expect(await seededCache.get('post_1')).toEqual({ title: 'cached' });
    });

    it('启用后应正常命中公共缓存', async () => {
        await clientConfig.set('cache.enabled', true);
        await cacheImpl.set('post_1', { title: 'cached' });

        const enabledCache = new CacheImpl(db as any, mockEnv, 'cache', 'database', clientConfig);
        expect(await enabledCache.get('post_1')).toEqual({ title: 'cached' });
    });
