...
function isAISummaryBackfillEligible(post: {
  draft: number;
  ai_summary: string;
  ...
  if (post.ai_summary.trim().length > 0) {
    return false;
  }

  return post.ai_summary_status !== "pending" && post.ai_summary_status !== "processing";
}

function isAISummaryForceBackfillEligible(post: {
  draft: number;
  ai_summary_status: string;
}) {
  if (post.draft === 1) {
...
export async function runCompatAISummaryBackfill(
  ...
  for (const item of items) {
    ...
    await syncPostAISummaryQueueState(db, serverConfig, env, item.id, {
      draft: false,
      updatedAt: item.updatedAt,
      resetSummary: true,
    });
    await clearPostCache(cache, item.id, item.alias, item.alias);
    queued += 1;
  }
...
export async function applyBlurhashCompatUpdate(db: DB, cache: CacheImpl, postId: number, content: string) {
