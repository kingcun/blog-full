...
describe("ConfigService", () => {
    ...
    describe("GET /queue-status - Queue status", () => {
        ...
        it("should return queue status for admin", async () => {
            sqlite.exec(`
                INSERT INTO posts (id, title, summary, ai_summary, ai_summary_status, ai_summary_error, content, listed, draft, top, uid)
                VALUES (1, 'Queued Post', '', '', 'pending', '', 'content', 1, 0, 0, 1)
            `);
    ...
    describe("Compatibility tasks", () => {
        ...
        it("should queue AI summary backfill for eligible posts", async () => {
            let sendCalls = 0;
            env.TASK_QUEUE = {
                send: async () => {
                    sendCalls += 1;
        ...
        it("should update blurhash metadata without resetting AI summary state", async () => {
            sqlite.exec(`
                INSERT INTO posts (id, title, summary, ai_summary, ai_summary_status, ai_summary_error, content, listed, draft, top, uid)
                VALUES (1, 'Blurhash Post', '', 'summary', 'completed', '', '![img](https://example.com/a.png)', 1, 0, 0, 1)
            `);
            ...
            expect(res.status).toBe(200);
            const row = sqlite.prepare("SELECT content, ai_summary, ai_summary_status FROM posts WHERE id = 1").get() as any;
    ...
    describe("Queue status actions", () => {
        it("should retry a failed queue task", async () => {
            ...
            sqlite.exec(`
                INSERT INTO posts (id, title, summary, ai_summary, ai_summary_status, ai_summary_error, content, listed, draft, top, uid)
                VALUES (1, 'Failed Post', '', '', 'failed', 'boom', 'content', 1, 0, 0, 1);
            `);
            ...
            const row = sqlite.prepare("SELECT ai_summary_status, ai_summary_error FROM posts WHERE id = 1").get() as any;
        ...
        it("should delete a completed queue task record", async () => {
            sqlite.exec(`
                INSERT INTO posts (id, title, summary, ai_summary, ai_summary_status, ai_summary_error, content, listed, draft, top, uid)
                VALUES (1, 'Completed Post', '', 'summary', 'completed', '', 'content', 1, 0, 0, 1);
            `);
