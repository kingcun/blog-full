...
describe('CommentService', () => {
    ...
    async function seedTestData(sqlite: Database) {
        ...
        sqlite.exec(`
            INSERT INTO comments (id, post_id, user_id, content, created_at) VALUES 
                (1, 1, 2, 'Comment 1 on post 1', unixepoch()),
                (2, 1, 2, 'Comment 2 on post 1', unixepoch()),
                (3, 2, 1, 'Comment on post 2', unixepoch())
        `);
    }

    describe('GET /:post - List comments', () => {
        ...
        it('should return empty array when post has no comments', async () => {
            ...
            sqlite.exec(`INSERT INTO posts (id, title, content, uid) VALUES (3, 'No Comments', 'Content', 1)`);
        ...
        it('should not expose sensitive fields', async () => {
            ...
            // Should not include postId and userId (excluded in query)
            expect(data[0]).not.toHaveProperty('postId');
    ...
    describe('POST /:post - Create comment', () => {
        it('should create comment with authenticated user', async () => {
            ...
            const comments = sqlite.prepare(`SELECT * FROM comments WHERE post_id = 1`).all();
