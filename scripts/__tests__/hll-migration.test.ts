...
describe('Migration Data Processing', () => {
    describe('IP List Processing', () => {
        ...
        it('should handle single post with multiple visits from same IP', async () => {
            // 模拟同一个 post 被同一个 IP 多次访问
        ...
        it('should handle single post with visits from different IPs', async () => {
            // 模拟同一个 post 被不同 IP 访问
            const ips = [
                '192.168.1.1',
                '192.168.1.2',
                '192.168.1.3',
...
describe('Real-world Migration Scenarios', () => {
    it('should handle typical blog post traffic pattern', async () => {
        // 模拟典型的博客 post 访问模式
    ...
    it('should handle high-traffic post with many unique visitors', async () => {
        const ips: string[] = [];
    ...
    it('should handle concurrent post processing simulation', async () => {
        // 模拟并发处理多个 post
        const posts = [
        ...
        const results = await Promise.all(
            posts.map(async (post) => {
                const hllData = await generateHLLData(post.ips);
                const uv = await estimateUV(hllData);
                return { postId: post.id, uv, pv: post.ips.length };
            })
        );
