...
describe('API Client', () => {
  ...
  describe('Post API', () => {
    it('should fetch post list', async () => {
      ...
      const result = await api.post.list({ page: 1, limit: 10 })

      expect(result.data).toEqual(mockResponse)
      expect(mockFetch).toHaveBeenCalledWith(
        expect.stringContaining('/api/post?page=1&limit=10'),
        expect.any(Object)
      )
    })

    it('should handle post list error', async () => {
      mockFetch.mockResolvedValueOnce(createMockResponse({
        ok: false,
        status: 500,
      ...
      const result = await api.post.list({ page: 1 })
    ...
    it('should fetch single post', async () => {
      const mockResponse = { id: 1, title: 'Post 1', content: 'Content 1' }
      ...
      const result = await api.post.get(1)
  ...
  describe('Tag API', () => {
    it('should fetch all tags', async () => {
      const mockResponse = [
        { id: 1, name: 'tag1', posts: 5 },
        { id: 2, name: 'tag2', posts: 3 },
      ]
    ...
    it('should fetch tag by name', async () => {
      const mockResponse = { id: 1, name: 'tag1', posts: [{ id: 1, title: 'Post 1' }] }
