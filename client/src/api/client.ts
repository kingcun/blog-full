...
class PostAPI {
  ...
  // GET /api/post
  async list(params?: { page?: number; limit?: number; type?: 'draft' | 'unlisted' | 'normal' }): Promise<ApiResponse<PostListResponse>> {
    ...
    return this.http.get<PostListResponse>(`/api/post${query ? `?${query}` : ""}`);
  }

  // GET /api/post/timeline
  async timeline(): Promise<ApiResponse<TimelineItem[]>> {
    return this.http.get<TimelineItem[]>("/api/post/timeline");
  }

  // GET /api/post/:id
  async get(id: number | string): Promise<ApiResponse<Post>> {
    return this.http.get<Post>(`/api/post/${id}`);
  }

  // POST /api/post
  async create(body: CreatePostRequest): Promise<ApiResponse<{ insertedId: number }>> {
    return this.http.post<{ insertedId: number }>("/api/post", body);
  }

  // POST /api/post/:id
  async update(id: number, body: UpdatePostRequest): Promise<ApiResponse<void>> {
    return this.http.post<void>(`/api/post/${id}`, body);
  }
  ...
  // GET /api/post/adjacent/:id
  async adjacent(id: number | string): Promise<ApiResponse<AdjacentPostResponse>> {
    return this.http.get<AdjacentPostResponse>(`/api/post/adjacent/${id}`);
  }

  // POST /api/post/top/:id
  async setTop(id: number, top: number): Promise<ApiResponse<void>> {
    return this.http.post<void>(`/api/post/top/${id}`, { top });
  }
}
...
class CommentAPI {
  ...
  async list(postId: number): Promise<ApiResponse<Comment[]>> {
    return this.http.get<Comment[]>(`/api/comment/${postId}`);
  }

  // POST /api/comment/:post
  async create(postId: number, body: CreateCommentRequest): Promise<ApiResponse<Comment>> {
    return this.http.post<Comment>(`/api/comment/${postId}`, body);
  }
...
class ConfigAPI {
  ...
  async applyCompatBlurhash(postId: number, content: string): Promise<ApiResponse<CompatBlurhashApplyResponse>> {
    return this.http.post<CompatBlurhashApplyResponse>(`/api/config/compat-tasks/blurhash/${postId}`, { content });
  }

  async retryQueueTask(postId: number): Promise<ApiResponse<QueueTaskActionResponse>> {
    return this.http.post<QueueTaskActionResponse>(`/api/config/queue-status/${postId}/retry`);
  }

  async deleteQueueTask(postId: number): Promise<ApiResponse<QueueTaskActionResponse>> {
    return this.http.delete<QueueTaskActionResponse>(`/api/config/queue-status/${postId}`);
  }
...
class SearchAPI {
  ...
  async search(keyword: string, params?: { page?: number; limit?: number }): Promise<ApiResponse<PostListResponse>> {
    const searchParams = new URLSearchParams();
    if (params?.page) searchParams.set("page", params.page.toString());
    if (params?.limit) searchParams.set("limit", params.limit.toString());

    const query = searchParams.toString();
    return this.http.get<PostListResponse>(`/api/search/${encodeURIComponent(keyword)}${query ? `?${query}` : ""}`);
  }
}
...
/**
 * RSS API methods - direct fetch for RSS posts
 */
class RSSAPI {
  constructor(private baseUrl: string) {}
