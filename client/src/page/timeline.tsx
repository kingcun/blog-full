...
export function TimelinePage() {
    const [posts, setPosts] = useState<Partial<Record<number, PostItem[]>>>()
        ...
        <>
            ...
            <Waiting for={posts}>
                <main className="w-full flex flex-col justify-center items-center mb-8 ani-show">
                    ...
                    {posts && Object.keys(posts).sort((a, b) => parseInt(b) - parseInt(a)).map(year => (
                        <div key={year} className="wauto flex flex-col justify-center items-start">
                            <h1 className="flex flex-row items-center space-x-2">
                                ...
                                <span className="text-sm t-secondary">
                                    {t('article.total_short$count', { count: posts[+year]?.length })}
                                    </span>
                            </h1>
                            <div className="w-full flex flex-col justify-center items-start my-4">
                                {posts[+year]?.map(({ id, title, createdAt }) => (
                                    <PostItem key={id} id={id.toString()} title={title || t('unlisted')}
...
export function PostItem({ id, title, createdAt }: { id: string, title: string, createdAt: Date }) {
    ...
    return (
        <div className="flex flex-row pl-8">
            ...
            <div className="flex-1 rounded-2xl m-2 duration-300 flex flex-row items-center space-x-4   ">
                ...
                <Link href={`/post/${id}`} target="_blank" className="text-base t-primary hover:text-theme text-pretty overflow-hidden">
                    {title}
                </Link>
            </div>
        </div>
