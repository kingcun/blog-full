...
import { POST_CARD_VARIANTS, normalizePostCardVariant } from "../components/post-card-options";
import { PostCardPreview } from "../components/post-card-preview";
import { _POST_LAYOUT_OPTIONS, normalizePostLayout } from "../components/post-layout-options";
...
export function Settings() {
  ...
  const postLayoutValue = normalizePostLayout(String(clientConfig.get("post.layout") ?? "list"));
  const postCardVariantValue = normalizePostCardVariant(String(clientConfig.get("post.card_variant") ?? "default"));
            ...
            <SettingsCard>
              ...
              <div className="mt-4 border-t border-black/5 pt-4 dark:border-white/10">
                <SettingsCardRow
                  header={
                    <SettingsCardHeader
                      title={t("settings.post_layout.title")}
                      description={t("settings.post_layout.desc")}
                    />
                  }
                  action={
                    <SearchableSelect
                      value={postLayoutValue}
                      onChange={(value) => {
                        setConfigValue("client", "post.layout", value);
                      }}
                      options={_POST_LAYOUT_OPTIONS.map((value) => ({
                        value,
                        label: t(`settings.post_layout.options.${value}`),
                      }))}
                      placeholder={t("settings.post_layout.title")}
                      emptyLabel={t("no_more")}
                      searchable={false}
                    />
                  }
              ...
              <div className="mt-4 border-t border-black/5 pt-4 dark:border-white/10">
                <SettingsCardRow
                  header={
                    <SettingsCardHeader
                      title={t("settings.post_card.title")}
                      description={t("settings.post_card.desc")}
                    />
                  }
                  action={
                    <SearchableSelect
                      value={postCardVariantValue}
                      onChange={(value) => {
                        setConfigValue("client", "post.card_variant", value);
                      }}
                      options={POST_CARD_VARIANTS.map((value) => ({
                        value,
                        label: t(`settings.post_card.options.${value}`),
                      }))}
                      placeholder={t("settings.post_card.title")}
                      emptyLabel={t("no_more")}
                      searchable={false}
                    />
                  }
                ...
                <SettingsCardBody>
                  <div className="grid gap-3 md:grid-cols-2">
                    {POST_CARD_VARIANTS.map((value) => (
                      <PostCardPreview
                        key={value}
                        variant={value}
                        selected={postCardVariantValue === value}
                        title={t(`settings.post_card.options.${value}`)}
                        description={t(`settings.post_card.preview.${value}`)}
                        onClick={() => {
                          setConfigValue("client", "post.card_variant", value);
                        }}
                      />
