...
async function publish({
  ...
  const t = i18n.t
  const { data, error } = await client.post.create(
    {
      title,
      alias,
      content,
  ...
  if (data) {
    showAlert(t("publish.success"), () => {
      Cache.with().clear();
      window.location.href = "/post/" + data.insertedId;
    });
  }
}
...
async function update({
  ...
  const t = i18n.t
  const { error } = await client.post.update(
    id,
    {
      title,
      alias,
...
export function WritingPage({ id }: { id?: number }) {
  ...
  useEffect(() => {
    if (!id) {
      if (title && !summary) {
        setSummary(title + "...");
      } else if (!title && summary && summary.endsWith("...")) {
        setSummary("");
      }
    }
  }, [title, id]);
